...
has Bool          $.go-back    = False; #= Whether the time zone's rules loop in the future. (set to false always in
...
method new (blob8 $tz, :$name) {
    ...
    if $*TZDEBUG {
        for @transition-times Z @transition-time-local-time-type-as-index
            -> ($time, $type) {
